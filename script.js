...
document.addEventListener("DOMContentLoaded", () => {
  ...
  const revealObserver = new IntersectionObserver((entries) => {
    entries.forEach(entry => {
      if (entry.isIntersecting) {
        ...
        if (entry.target.querySelector(".stat-val")) {
          triggerStatsCounter();
        }
      }
    });
  ...
  /* ====================================================
     9. INTERACTIVE MCQ PLAYABLE WIDGET
     ==================================================== */
  const quizQuestions = [
    {
      ...
      correct: 2,
      explanation: "Mitochondria release energy in the form of ATP, making them the powerhouse."
    },
    {
      question: "What is the chemical formula of common household salt?",
      options: ["H₂O", "NaCl", "CO₂", "NaOH"],
      correct: 1,
      explanation: "Sodium Chloride (NaCl) is the chemical compound representing table salt."
    },
    {
      question: "Which essential gas do plants release during photosynthesis?",
      options: ["Oxygen", "Carbon Dioxide", "Nitrogen", "Hydrogen"],
      correct: 0,
      explanation: "Plants take in Carbon Dioxide and release Oxygen into the atmosphere."
    }
  ];
  ...
  function showQuizCompletionScreen() {
    ...
    let trophy = "🥇";
    let rating = "Brilliant!";
    if (userScore === 2) { trophy = "🥈"; rating = "Great Job!"; }
    ...
    quizCard.innerHTML = `
      <div class="quiz-result-view">
        ...
        <div class="q-res-score">Your Score: <span>${userScore}/${quizQuestions.length}</span></div>
        ...
        <button class="btn-main" onclick="restartQuizGame()" style="padding: 10px 24px; font-size: 0.9rem; border: none; cursor: pointer; margin-top: 15px;">
          Try Again 🔄
        </button>
      </div>
    `;
  ...
  /* ====================================================
     10. EXPANDED TESTIMONIALS SLIDER
     ==================================================== */
  ...
  /* ====================================================
     13. IMAGE LIGHTBOX MODAL VIEWER
     ==================================================== */
  ...
  zoomImages.forEach(img => {
    img.addEventListener("click", () => {
      if (lightboxModal && lightboxImg) {
        ...
        if (lightboxCap) {
          lightboxCap.textContent = img.alt || "Vidya Topper App Screen View";
        }
  ...
  /* ====================================================
     14. CINEMATIC HERO TITLE STAGGERED RISE
     ==================================================== */
  ...
  if (heroTitle) {
    ...
    heroTitle.childNodes.forEach(node => {
      ...
      } else if (node.nodeType === Node.ELEMENT_NODE && node.classList.contains('grad')) {
        const mask = document.createElement('span');
        mask.className = 'hero-word-mask';
        mask.style.overflow = 'visible';
        const el = document.createElement('span');
        el.className = 'hero-word-el grad';
    ...
    heroTitle.innerHTML = '';
    newNodes.forEach(node => heroTitle.appendChild(node));
    
    const elements = heroTitle.querySelectorAll('.hero-word-el');
